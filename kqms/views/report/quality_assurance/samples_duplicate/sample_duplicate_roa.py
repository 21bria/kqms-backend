...
def chart_wet_year_roa(request):
    ...
    # Kondisi WHERE berbeda berdasarkan vendor
    if db_vendor == 'postgresql':
        where_clause = "WHERE EXTRACT(YEAR FROM release_date) = %s"
    elif db_vendor in ['mssql', 'microsoft', 'mysql']:
        where_clause = "WHERE YEAR(release_date) = %s"
    else:
        raise ValueError("Unsupported database vendor.")

    # Query utama (tanpa WHERE)
    base_query = """
           SELECT
    ...
    # Gabungkan base query dengan where_clause
    sql_query = f"{base_query} {where_clause}"


    # df = pd.read_sql_query(query, connection)
    df = pd.read_sql_query(sql_query, connections['kqms_db'], params=(filter_year,))
...
def chart_wet_month_roa(request):
  # Mendapatkan nilai filter dari request
    ...
    # Tentukan WHERE clause berdasarkan vendor
    if db_vendor == 'postgresql':
        where_clause = "WHERE EXTRACT(YEAR FROM release_date) = %s AND EXTRACT(MONTH FROM release_date) = %s"
    elif db_vendor in ['microsoft', 'mssql', 'mysql']:
        where_clause = "WHERE YEAR(release_date) = %s AND MONTH(release_date) = %s"
    else:
        raise ValueError("Unsupported database vendor.")
    ...
    # Gabungkan dengan WHERE clause
    sql_query = f"{base_query} {where_clause}"

    # Eksekusi query
    params = (filter_year, filter_month)
    df = pd.read_sql_query(sql_query, connections['kqms_db'], params=params)
