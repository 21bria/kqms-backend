...
# For Chart
def get_chart_detail_topsoil(request):
    ...
    if filter_type == 'monthly' and filter_year and filter_month:
        ...
        return get_weekly_detail_chart()
    ...
    query = """ 
            ...
            LEFT JOIN (
			    SELECT 
			        t_load,
			        SUM(CASE 
					    WHEN nama_material IN ('Top Soil') 
					    THEN tonnage 
					    ELSE 0 
    ...
    query = """
        SELECT 
            ...
            LEFT JOIN (
                SELECT 
                    left_date,
                    SUM(CASE WHEN nama_material = 'Top Soil' THEN tonnage ELSE 0 END)::numeric AS tonnage
                FROM mine_productions
            ...
            LEFT JOIN (
                SELECT 
                    ...
                    SUM(COALESCE(topsoil,0)) AS plan_data
...
def get_weekly_detail_chart(filter_week):
    ...
    topsoil_cols = ['topsoil']
    topsoil_plan_cols = [f + '_plan' for f in topsoil_cols]

    # Konversi kolom ke numerik (handle string atau null)
    for col in topsoil_cols  :
        df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0).astype(float)

    df['topsoil']       = df[topsoil_cols].sum(axis=1)
    df['topsoil_plan']  = df[topsoil_plan_cols].sum(axis=1)
...
def get_all_chart():
    query = """ 
             SELECT 
                TO_CHAR(mine_productions.date_production, 'YYYY') AS tahun,
                SUM(mine_productions.tonnage) AS total_tonnage,
                SUM(
                    COALESCE(plan_productions.topsoil, 0) + COALESCE(plan_productions.ob, 0) + 
                    COALESCE(plan_productions.lglo, 0) + COALESCE(plan_productions.mglo, 0) +
                    COALESCE(plan_productions.hglo, 0) + COALESCE(plan_productions.waste, 0) +
                    COALESCE(plan_productions.mws, 0) + COALESCE(plan_productions.lgso, 0) +
                    COALESCE(plan_productions.mgso, 0) + COALESCE(plan_productions.hgso, 0) +
                    COALESCE(plan_productions.quarry, 0) + COALESCE(plan_productions.ballast, 0) + 
                    COALESCE(plan_productions.biomass, 0)
                ) AS plan_data
            FROM mine_productions
            LEFT JOIN plan_productions 
                ON mine_productions.date_production = plan_productions.date_plan
            GROUP BY TO_CHAR(mine_productions.date_production, 'YYYY')
            ORDER BY tahun;
    """
