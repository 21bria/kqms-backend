...
# For Chart
def get_chart_detail_waste(request):
...
def get_weekly_detail_chart(filter_week):
    ...
    waste_cols = ['waste']
    waste_plan_cols = [f + '_plan' for f in waste_cols]

    # Konversi kolom ke numerik (handle string atau null)
    for col in waste_cols  :
        df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0).astype(float)

    df['waste']       = df[waste_cols].sum(axis=1)
    df['waste_plan']  = df[waste_plan_cols].sum(axis=1)
