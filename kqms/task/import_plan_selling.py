...
@shared_task(name='kqms.task.import_plan_selling.import_plan_selling')
def import_plan_selling(file_path, original_file_name):
    ...
    try:
        with transaction.atomic():
            for index, row in df.iterrows():
                ...
                if SellingPlan.objects.filter(plan_date=plan_date,type_ore=type_ore).exists():
        ...
        # Simpan batch data
        SellingPlan.objects.bulk_create(list_objects, batch_size=200)
    ...
    try:
            ...
            task_id=import_plan_selling.request.id,
